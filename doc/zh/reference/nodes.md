...
| `detour` | string? | null | 前置节点名称。由 dae `exit -> front` 链填充（前置跳为内部节点），或经结构化/导入面按名称设置。见[出站链式代理](#出站链式代理exit---front) |
...
对于写成 `exit -> front -> ...` 的链，会解析每一跳：最左节点是出口，其右侧每个节点都是抵达它的 dial-proxy，与 dae 的 `NewNetproxyDialerFromLink` 一致。自动名称只来自解码后的 `#fragment`、VMess `ps` 或 `{scheme}-{host}`；解析器绝不以原始 URI 或 userinfo 作为回退，因此生成名称不会泄漏凭据。显式 tag、fragment 与 `ps` 值仍由用户控制。

## 出站链式代理（`exit -> front`）

节点可以经由一个或多个前置节点连接自身服务器，而不是直接建立 TCP 连接。dae 箭头形式先写出口，随后写它的 dial-proxy 各跳：
...
`->` 的每一侧都是分享链接，与 dae 完全一致：`exit: 'trojan://... -> socks5://... -> socks5://...'` 会先经第一个 SOCKS5 跳、再经第二个抵达 Trojan 服务器。被 tag 的节点是出口；每个前置跳都会成为内部节点（名称 `chain-<完整内容 id>`），不参与组候选，也不会出现在 Clash API 与 `honk-tool` 中。共享同一前置链接的两条链会复用同一个前置节点。当已声明节点或订阅条目与某个前置跳具有相同的内容身份时（例如订阅把同一台服务器既单独列出、又作为某条链的前置），先被接纳的条目保留、后到的重复项被丢弃；出口仍能抵达被保留的节点，并复用它作为前置。
