...
不带 tag 且不含 `://` 的文本会被忽略。带显式 tag 的条目仍交给配置验证，要求名称非空且 URL 使用 `http://`、`https://` 或 `file:`。`http-file://` 和 `https-file://` 仍不受支持。

`file:` 订阅从本地读取正文，而不是发起网络请求。`file:///etc/honk/local.sub` 与 `file://localhost/etc/honk/local.sub` 是绝对路径；dae 的旧式 `file://relative/path/to/mysub.sub` 是相对路径，以 URL authority 作为第一段，并相对于配置文件所在目录解析（配置没有文件时——例如从字符串解析的配置——相对进程工作目录）。带凭据、query、fragment 或没有文件名的 URL（`file://`、`file:///`）会在验证阶段被拒绝，因此通过验证的 `file:` URL 一定指向一个本地文件。绝对 `file:` URL 没有主机名，不带 tag 的条目会得到空名称，因此仍需提供 tag。本地正文与下载正文经过完全相同的解析与持久化流程；它不使用网络、重定向，也不使用配置的 User-Agent 和 headers。
...
| `url` | string | `""` | 是 | HTTP(S) 拉取 URL，或从本地磁盘读取的 `file:` URL。 |
...
`honk-tool sub` 的下载正文与本地文件使用同一个解析器，`file:` URL 也走与 runtime 相同的分类逻辑。

## 拉取、持久化与恢复

`global.store_subscribe` 默认为 `true`。启用后，runtime 会打开私有订阅存储并立即拉取每个已启用订阅。请求默认使用 `honk/<version>`，可由 `user_agent` 覆盖。非零 `update_interval` 会安排后续刷新。本地 `file:` 读取不发送请求，因此其 `user_agent` 与 `headers` 只参与 fetch 身份，不参与正文读取；读取按进程的普通文件系统权限进行并跟随符号链接，存储目录的属主、no-follow 与权限收紧规则只覆盖存储目录及其缓存文件。
...
| 写入边界 | HTTP 成功或本地读取且正文通过导入校验后，保存完整原始正文，包括被拒绝的条目。相对保留目录描述符创建的临时文件完成 sync 后在同一目录内原子 rename，随后对目录执行 sync。 |
| 重定向 | 最多 5 跳。从 `https` 重定向到其他 scheme 会让本次拉取失败；重定向到配置 URL 自身未使用的回环、私有、链路本地或未指定字面地址同样失败。解析到这类地址的主机名不在检测范围内。`file:` 读取没有重定向或地址策略。 |
...
- HTTP、本地读取、UTF-8 编码、解析或无可用节点错误不会发布替换节点，也不会写入，因此活动节点与上一次有效正文都会保留。通过校验的正文按原始字节保存，不修复编码。
